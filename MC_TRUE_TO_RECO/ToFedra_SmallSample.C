...
int ToFedra_SmallSample(){
    ...
    for (int i = minplate; i <= nplates; i++){
        cout << "Plate: " << i+1 << endl;
        ect[i] = new EdbCouplesTree();
        ect[i]->InitCouplesTree("couples",Form("b%06d_SmallSample/p%03d/%d.%d.0.0.cp.root",idbrick, i+1,idbrick, i+1),"RECREATE");
        vec_ect.push_back(ect[i]);
        //delete ect;
    }
    bool savehit;
    ...
    for(int iev = 0; iev<nevents; iev++){ //nevents //19375

        if (iev%1000==0) cout << " Completed " << 100.*iev/nevents << " % " << endl;
        EventTree->GetEntry(iev);
        //cout << " iev " << iev << endl;
        //if (iev != 9832) continue;
        
        try { vt = evt->GetVolumeTrack(0); }
        catch (...) { continue; }
