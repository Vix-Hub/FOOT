...
#define DEBUG_VTXID  6864 //447 //10464 //6557  //88
...
#define N_STACKS 7
#define DEBUG_S0_PLATE_END 9 //16
#define DEBUG_S0_ID_END 4021 //563496
#define DEBUG_S0_PLATE_ST 31
#define DEBUG_S0_ID_ST 4038
...
float Z_LAYER[NPLATE+1]={0}; // z of each plate   <- 0 = 0
...
int postvertex3_new_temp()
{
    ...
    if(FAST!=1) cout << "At the end I have " << final_varr->GetEntries() << " vertices" << endl; //"\tTime: " << t_tot.RealTime() << " s\t" << t_tot.RealTime()/60 << " min " << endl;
...
TObjArray* AnalyseFakeVtxs(TObjArray &arrv, EdbVertexRec *vrec){
    ...
    for(int iv=0; iv<nv; iv++){
        ...
        if((FAST==4||FAST==100) && DEBUG_VTXID!=-99){ //TAGLIO CUT FAST 2
            if(vID!=DEBUG_VTXID) {
                vertex->SetFlag(-99);
                continue;
            } 
                ...
                if(First_N>3){
                    if(vertex->GetVTa(0)->Zpos()!=vertex->GetVTa(1)->Zpos() && TMath::Abs(First_Theta-Second_Theta)<0.1){
                        ...
                        if(1==1){
                            ...
                            if(v1){
                                v1=vrec->AddTrackToVertex2(v1, firsttrack,vertex->GetVTa(0)->Zpos());
                                if(v1){
        ...
        if(vertex->Flag()!=-99){
            for (int itrk = 0; itrk < vertex->N(); itrk++){
                ...
                if(track->GetSegmentFirst()->Z()<vertex->VZ() && track->GetSegmentLast()->Z()>vertex->VZ()){
                    COUNT_STRANGE++;
                    vertex->SetFlag(-99);
					if(EVERBOSE==100 && (vID==DEBUG_VTXID||vertex->GetTrack(0)->MCEvt()==DEBUG_MCEVT)) cout << "vertex deleted rigo 935" << endl;
...
void FillVtxPlate(TObjArray &arrv){
    for(int i=PLMIN; i<=BRAGGPLATE; i++) vtxPat[i] = new TObjArray();
...
EdbVertex *EdbVertexRec::AddTrackToVertex2(EdbVertex *eVertex, EdbTrackP *eTr, int zpos){
    EdbVTA *vta = 0;
    EdbVertex *old = 0;
                ...
                if(vertex->N()>2){
                    for(int itrk=0; itrk<vertex->N(); itrk++){
                        ...
                        if(vta->GetTrack()->Track()==vta1->GetTrack()->Track()){
                            ...
                            EdbVertex *vertex1 = vrec->RemoveVTAFromVertex(*vertex, *vta1);
                            if (FAST==4 && EVERBOSE==100) cout << " vertex ID After RemoveVTA " << vertex->ID() << " VZ " << vertex->VZ() << " Z " << vertex->Z() <<  endl;
                            if (FAST==4 && EVERBOSE==100) cout << " vertex1 ID After RemoveVTA " << vertex1->ID() << " VZ " << vertex1->VZ() << " Z " << vertex1->Z() <<  endl;
        ...
        if( mergedvtx->V() )
            if( mergedvtx->V()->valid() ) {
                ...
                if( mergedvtx->V()->prob() >= -1 )   // accept new N-tracks vertex
                {
                    ...
                    if(EVERBOSE==9 ||( EVERBOSE==100 && mergedvtx->ID()==DEBUG_VTXID )) cout << "merged vtx: " << mergedvtx->ID() << "\t" << mergedvtx->VX() << "\t" << mergedvtx->VY() << "\t" << mergedvtx->VZ() << "\t" << Get_vtx_plate(vtx->VZ()) << "\t" << mergedvtx->Flag() << "\t" << mergedvtx->N() << endl;
