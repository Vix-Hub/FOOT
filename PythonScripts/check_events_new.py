...
def FindClosestMatch(event_couples, s0x, s0y, s0z, s0plate, couple, EVERBOSE):
...
for track in tracks:
    ...
    track_couples.append((track.s[0].Plate(), track.s[0].ID(), track.s[0].X(), track.s[0].Y(), track.s[0].Z(), track.s[track.nseg-1].X(), track.s[track.nseg-1].Y(), track.s[track.nseg-1].Z(), track.s[track.nseg-1].TX(), track.s[track.nseg-1].TY(), track.s[track.nseg-1].Plate()))
t1 = time.time()    
