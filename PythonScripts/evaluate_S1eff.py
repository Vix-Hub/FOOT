...
TrackFile = r.TFile("b000111.1.0.0.trk.root", "READ")
tracks = TrackFile.Get("tracks")

outfile = r.TFile("S1_eff.root", "RECREATE")
tup = r.TNtuple("tup", "S1 efficiency check", "exp_segs:segs:plate")
...
for track in tracks:
    counter +=1
    if track.nseg<15 or track.s[0].Theta()<0.03:
        continue
...
for i in range(len(r0_plates)):
    ...
    if (i!=0 and i!=len(r0_plates)-1):
        efficiency.append(float(r0_total[i])/float(r0_expected[i]))
