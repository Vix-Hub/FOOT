...
InFileName = "vertices_improved_fast_3_new_temp.root"
...
OutFileName = "vertices_improved_fast_3_new_temp_2.root"
...
print(" Saved Vtx Tracks ")

# some vertices contain the same track twice (or more than twice) -> remove them
for i in range(n_vertices):
	vertex = vrec.eVTX.At(i)
	...
	if (EVERBOSE==100 and vertex.ID()==6810):
		print("VTXTRKLIST")
		print(vtx_trk_list)
		print("\n")

	unique_tracks = np.unique(vtx_trk_list)

	for ttrack in unique_tracks:
		if (vtx_trk_list.count(ttrack)>1):
			indices = [i for i, x in enumerate(vtx_trk_list) if x == ttrack] #find all positions of same track in list
			for irem in range(len(indices)-1):
				vta_to_remove = vertex.GetVTa(pos_list[indices[irem]])
				#if (EVERBOSE==100 and vta_to_remove):
				#	print("vertex ID " + str(vertex.ID()) + " N " + str(vertex.N()) + " vta_to_remove " + str(vta_to_remove))
				if (vta_to_remove):
					vertex.RemoveVTA(vta_to_remove)
				if (EVERBOSE==100 and vertex.ID()==6810):
					print(" Removed " + str(ttrack))
