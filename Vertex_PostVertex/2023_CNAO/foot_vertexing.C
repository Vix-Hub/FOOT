...
/*
 scp /Users/Giuliana/Desktop/Uni/FOOT/TEST_RECO_MIC/foot_vertexing.C scanner@nusrv9.na.infn.it:/home/scanner/foot/RECO_MC_TEST_giul/b000003/
 
 scp /Users/Giuliana/Desktop/Uni/FOOT/TEST_RECO_MIC/foot_vertexing.C scanner@nusrv9.na.infn.it:/home/scanner/foot/2019_GSI/GSI3/b000333
 
 scp /Users/Giuliana/Desktop/Uni/FOOT/TEST_RECO_MIC/foot_vertexing.C scanner@nusrv9.na.infn.it:/home/scanner/foot/2019_GSI/GSI2/b000222
...
//ATTENZIONE BISOGNA MODIFICARE DZmax a seconda del brick
...
{
  float DZmax =4200; //GSI1 era 3600 //GS3 5600; // GSI1= 4200; //GSI2 = 7200
       // if(BRICKID==2||BRICKID==222 || BRICKID==4||BRICKID==444) DZmax = 7200;
        //else if(BRICKID==1||BRICKID==111 || BRICKID==3||BRICKID==333) DZmax=4200;
        //4200 (GSI1);//7200. (GSI2);
   float ProbMinV   = 0.01; //0.001 // minimum acceptable probability for chi2-distance between tracks
    //float ProbMinV   = 0.01;
    float ImpMax     = 30; //70.;    // maximal acceptable impact parameter [microns] (for preliminary check)
...
//---------------------------------------------------------------------
void foot_vertexing(char *dset=0)
{
    //trvol(dset);
    ...
    trvol(dset,"nseg>=5 && s[0].eY>40000 && s[0].eY<60000 && s[0].eX>35000 && s[0].eX<60000 "); //&& s.eY>25000 && s.eY<75000 && s.eX>35000 && s.eX<90000
//&& (s[0].Theta()<0.2 || s[0].DZ()>3000*(1+TMath::Exp(0.7*s[0].Theta()*s[0].Theta()) ) ) && s[0].Flag()>-1
//&& (s[0].Theta()<0.2 || s[0].DZ()>2000*(1+TMath::Exp(2.0*s[0].Theta()*s[0].Theta()) ) ) && s[0].Flag()>-1
...
void set_segments_dz(float dz)
{
    ...
    for(int i=0; i<np; i++) {
        ...
        if(!p) {
            ...
            float zmissingPID = -10000; //note, set->eB->GetPlate(i) uses the PID, set->GetPlate(i) uses the number of plate, be careful!
            
			if (i >= 3) zmissingPID = set->eB.GetPlate(i-3)->Z();
			EdbPattern *pat = new EdbPattern( 0., 0.,zmissingPID);
			cout << " added pattern with z " << zmissingPID << endl;
            pat->SetID(i);
            pat->SetScanID(0);
        ...
        for(int j=0; j<ns; j++) {
            EdbPattern *p = gAli->GetPattern(i);
            p->GetSegment(j)->SetDZ(dz);
        }
    }

	cout << " end of set segments dz " << endl;
}
...
void do_vertex()
{
    ...
    TFile *fvtx = new TFile("vertices.root","RECREATE"); //OUTPUT FILE NAME
    ...
    for(Int_t ivtx=0; ivtx<gEVR->eVTX->GetEntries(); ivtx++){
        ...
        if(vplate>30) cout << "azz " << ivtx << "\t" << vID << "\t" << vplate << "\t" << vz << "\t" << n << endl;
        v_flag=vertex->Flag();
		...
		EdbVertex *vclone = (EdbVertex*) vertex->Clone();
		varr->Add(vclone);
...
void get_z_pattern_old()
{

	cout << " here get z pattern old " << endl;
    ...
    int np = gAli->Npatterns();
    int layer=np;
    cout << " N patterns available " << np << endl;
    
    for(int i=0; i<LASTLAYER; i++) {
        EdbPattern *p = gAli->GetPattern(i);
		if (p) cout << " Here Pattern " << p->ID() << " Z " << p->Z() << " p->N() " << p->N()<<  endl;
...
void get_z_pattern_old2()
{
    
    int np = gAli->Npatterns();
    
    TFile *setfile = TFile::Open(Form("b%06d.0.0.0.set.root",BRICKID));
    cout << "I'm checking patterns with set " << "b" << BRICKID << ".0.0.0.set.root" << "\t np: " << np << "\tRunning on " << LASTLAYER << " plates" << endl;
    EdbScanSet *set = (EdbScanSet*) setfile->Get("set");
    ...
    cout << "New n patters: " << gAli->Npatterns() << endl;
}
...
int Get_vtx_plate(float vz){
    ...
    for(int layer=1;layer<=LASTLAYER;layer++){
		...
		if(vz>Z_LAYER[25]&&vz<Z_LAYER[27]){
            	cout << "vz: " << vz << "\tlayer " << layer << "\tmin " << Z_LAYER[layer] << "\t" << Z_LAYER[layer+1] << endl;
            	return 27;
			}
		}		       
