...
#define ALLOW_LONGER_GAPS 0
#define UNISCI_TRACCE_LARGER_CUT 0
int LASTLAYER[N_STACKS+1]={1,33,71,76,83,90,110,120}; //CN7
//int LASTLAYER[N_STACKS+1]={1,30,66,76,83,90,120,140}; //esposizione Oxy@400MeV/n 2019


const int NPLATE = LASTLAYER[N_STACKS];
#define NPLATE_1 121
const int PLMIN=3;//LASTLAYER[0];
const int PLMAX=69; // per tutto brick LASTLAYER[N_STACKS] //conta da 0
...
const float xmax = 60000;//125000;
const float ymin = 40000;//0;
const float ymax = 60000;//100000;
...
int postvertex3_new()
{
    ...
    if (FAST==1 || FAST==100) inputfile_vtx = TFile::Open("vertices.root", "READ");
    ...
    for(int iv=0; iv<arrVTX->GetEntries(); iv++){
        EdbVertex *vertex = (EdbVertex*)(arrVTX->At(iv));
	cout << " start of code vertex prob before reset " << vertex->V()->prob() << endl;
        vertex->ResetTracks();
	cout << " vertex probability " << vertex->V()->prob() << endl; 
    }
    
    EdbScanCond *myscancond = new EdbScanCond();
    ali->SetScanCond(myscancond);
    EdbDataProc *dproc = new EdbDataProc();
    TCut tracksel("s[0].X()>=35000 && s[0].X()<60000 && s[0].Y()>40000 && s[0].Y()<60000");//"t.X()>40000&&t.X()<85000 && t.Y()>36000&&t.Y()<62000"); //AREA SEGNALE
	...
	cout << " Final _varr entries " << final_varr->GetEntries();
    ...
    if(FAST!=2 && FAST==3){
        fvtx->cd();
        //mygEVR->Write();
	cout << " written gEVR " << endl;
	final_varr->Write("varr");
        //new_vtxtree->Write();
...
TObjArray* AnalyseFakeVtxs(TObjArray &arrv, EdbVertexRec *vrec){
	...
	//cout << " Analyze Fake vertex prob before reset " << vertex->V()->prob() << endl;
        vertex->ResetTracks();
	//cout << " Analyze Fake vertex probability " << vertex->V()->prob() << endl; 
    }
...
TObjArray *FindCloseTracks(TObjArray *varr, EdbVertexRec *vrec){
    ...
    for(int ipl = PLMIN; ipl<=BRAGGPLATE; ipl++){ //BRAGGPLATE
        ...
        for(int iv=0; iv<nv; iv++){
            ...
            for(int iipl=ipl-2; iipl<ipl+5; iipl++){
