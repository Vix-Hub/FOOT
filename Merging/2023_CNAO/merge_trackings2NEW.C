...
int LASTLAYER[NSTACK+1]={2,33,71,76,83,90,110,140}; //CN7
void TransformAngles(EdbSegP &segment, EdbAffine2D aff);

void merge_trackings2NEW(){
    
    cout << endl << endl << endl << "Brick: " << IDBRICK << endl;
    
    folderpath = "./";
    
    TString newpath=Form("%sb%06d/b%06d.0.%d.%d.trk.root", folderpath.Data(), IDBRICK, IDBRICK, S0, SL);
...
void add_stack(int S)
{
    ...
    if(S>1){
        ...
        for(int ipid=0; ipid<scanset->Brick().Npl(); ipid++){
            cout << ipid << "\t" << scanset->GetID(ipid)->GetPlate() << "\t" << LASTLAYER[S-1]-2 << endl;
    ...
    for (int itrk = 0; itrk < tracks->GetEntries(); itrk++){ //tracks->GetEntries()
        ...
        for(int iseg=0; iseg<nseg; iseg++){
            ...
            if(DIRECTION==1){
				...
				//cout << " Segment Coordinates before Transform " << seg->X() << " " << seg->Y() << " " << seg->TX() << " " << seg->TY() << endl;
                seg->Transform(aff);
	            //cout << " Segment Coordinates after Transform XY " << seg->X() << " " << seg->Y() << " " << seg->TX() << " " << seg->TY() << endl;
                
				TransformAngles(*seg, *affTXTY);
                //cout << " Segment Coordinates after Transform TXTY " << seg->X() << " " << seg->Y() << " " << seg->TX() << " " << seg->TY() << endl;
                Zseg = p->Z()+seg->Z();
                    if(PID_NOHOLE==1){
