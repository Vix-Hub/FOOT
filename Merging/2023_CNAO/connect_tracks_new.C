...
#define BEAM_PLATES -30 //negative value = disabled 
#define BEAM_THETA 0.03 //negative value = disabled
#define USE_JUMP_CUTS 1 //use larger cuts if there are missing plates

// Sections to Merge, from 1 to 7
const int S0 = 1;
const int SL = 2;
const int LASTLAYER[NSTACKS+1]={1,33,71,76,83,90,110,120}; //CN7, first number is not used
...
// Merge Cuts
float B_MAX = 50; //micron
float DT_MAX = 0.05; // rad

float B_MAX_JUMP = 70; //micron 
float DT_MAX_JUMP = 0.070; //rad
...
// X-Y Cut
const float xmin = 15000;//0;
const float xmax = 85000;//125000;
const float ymin = 15000;//0;
const float ymax = 85000;//100000;

const int PLMIN=0;  // First Brick Plate
const int PLMAX=LASTLAYER[SL]-2;  // Last Brick Plate
...
int connect_tracks_new(int IS_SECOND_STEP=0) {

    int MC = 0;
    if (IDBRICK < 100) MC = 1;

    TString file_name = Form("b%06i.0.%i.%i.trk_trasl_step_2.root", IDBRICK, S0, SL);
    if (IS_SECOND_STEP == 1) file_name = Form("b%06i.0.%i.%i.trk_merged_new.root", IDBRICK, S0, SL);

    /*if (IS_SECOND_STEP) {
        B_MAX = 150; DT_MAX = 0.1;
        file_name = Form("b%06i.0.%i.%i.trk_merged_new.root", IDBRICK, S0, SL);  
    }*/

    cout << " Using IS_SECOND_STEP = " << IS_SECOND_STEP << endl;
    ...
    if (IS_SECOND_STEP) merge_file_name_temp = Form("b%06i.0.%i.%i.trk_merged_temp_2.root", IDBRICK, S0, SL);
    if (EVERBOSE==100||EVERBOSE==101) merge_file_name_temp = Form("b%06i.0.%i.%i.trk_merged_EVERBOSE.root", IDBRICK, S0, SL);
    ...
    for (int iipl=LASTLAYER[0]+1; iipl<=PLMAX; iipl++) { //LASTLAYER[0]+1

        for (int icheck = 0; icheck < 3; icheck++) {
            if (iipl == MISSING_PLATES[icheck]) continue;
        }
	    ...
	    float start_theta = start_trk->GetSegmentFirst()->Theta();
	        ...
	        if (start_plate<=BEAM_PLATES && start_theta<=BEAM_THETA && BEAM_PLATES>=0) search = 0;
            if (IS_SECOND_STEP==0 && start_theta<=BEAM_THETA) search = 0;
            if (IS_SECOND_STEP && start_theta > BEAM_THETA) search = 0;
...
void FillZ_LAYER_SET() {
    ...
    if (!setfile || setfile->IsZombie()) {
        cerr << "Error: Cannot open set file!" << endl;
        return;
    }

    EdbScanSet *set = (EdbScanSet*) setfile->Get("set");
    if (!set) {
        cerr << "Error: 'set' object not found in file!" << endl;
        return;
    }

    for (int i = 0; i < PLMAX; i++) {
        EdbPlateP *plate = set->eB.GetPlate(i);
        if (!plate) {
            cerr << "Warning: Plate " << i << " is missing in the ScanSet!" << endl;
            Z_LAYER[i] = -999999;  // Or another sentinel value indicating missing
            continue;
        }

        Z_LAYER[i] = plate->Z();
        cout << "Layer: " << i << " Z = " << Z_LAYER[i] << endl;
    }
}
...
void checkpatterns(){
    ...
    int j=0;
    for(int i=0; i<PLMAX; i++) {
        EdbPlateP *plate = set->eB.GetPlate(i); //note, set->eB->GetPlate(i) uses the PID, set->GetPlate(i) uses the number of plate, be careful!
        
        if (!plate) {
        cout << "Plate " << i << " is missing in the set, skipping..." << endl;
        continue;
        }
        float zmissingPID = plate->Z();
...
void FillTracksCells(TObjArray &arrt){
    ...
    for(int itr=0; itr<ntr; itr++){
        ...
        if (EVERBOSE==100 && plate_tr==DEBUG_S0_PLATE_S1 && t->GetSegmentFirst()->ID()==DEBUG_S0_ID_S1) cout << " Adding S1 Candidate to grid with plate_tr " << plate_tr << endl;
    }
...
EdbTrackP* FindClosestCandidate(int nplates, EdbTrackP* start_trk, TClonesArray *segments_new, TClonesArray *fitted_segments_new, const double MAX_B, int &added_segs, const double MAX_DT) {
    ...
    float B_MAX_INT = MAX_B; //copy of the value, can be modified in the case of jumps
    float DT_MAX_INT = MAX_DT; 
    ...
    { 
        for (int ipl=s0_plate+1; ipl<=s0_plate+nplates; ipl++) {  //look for candidates in 4 plates
            ...
            B_MAX_INT = B_MAX; DT_MAX_INT = DT_MAX; //start with nominal values

            for (int icheck = 0; icheck < 3; icheck++) {
                if (ipl == MISSING_PLATES[icheck]) { nplates+=1; continue; } //add 1 plate if one is among the missing
                if (ipl - 1 == MISSING_PLATES[icheck] && USE_JUMP_CUTS) { B_MAX_INT = B_MAX_JUMP; DT_MAX_INT = DT_MAX_JUMP; }
                //else { B_MAX_INT = B_MAX; DT_MAX_INT = DT_MAX; }
            }

            //if (ipl - 1 == MISSING_PLATES[0]) cout << " here candidate in plate " << ipl << " using BMAX_INT " << B_MAX_INT << " DT_MAX_INT " << DT_MAX_INT << endl;
            ...
            for (int itrk1=0; itrk1<n_trk_cell; itrk1++) {
                ...
                b_back = CalcDist(end_segf->X(), end_segf->Y(), end_segf->Z(), start_segf->X(), start_segf->Y(), start_segf->Z(), end_segf->TX(), end_segf->TY());
                b = CalcDist(start_segf->X(), start_segf->Y(), start_segf->Z(), end_segf->X(), end_segf->Y(), end_segf->Z(), start_segf->TX(), start_segf->TY());
                ...
                if (b<B_MAX_INT && TMath::Abs(dtx)<DT_MAX_INT && TMath::Abs(dty)<DT_MAX_INT) {
    ...
    if (sorted_IPs_for.size()>0) {  //if at least one candidate was found
        for (int icand=0; icand<sorted_IPs_for.size(); icand++) {
            ...
            if (to_merge_trk->Flag()!=300 && current_b<B_MAX_INT) { //if the candidate was not used, the cut was added before
