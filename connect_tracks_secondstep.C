...
TObjArray *arrTRK = new TObjArray();   // original tracks not already merged in step 1
...
EdbTrackP* FindClosestCandidate(const int nplates, EdbTrackP* start_trk, TClonesArray *segments_new, TClonesArray *fitted_segments_new, const double MAX_B, int &added_segs, const double MAX_DT) {
    ...
    float b=0, r=500., b_back=0, dtheta=0, dtx=0, dty=0;
    ...
    for (int ipl=(s0_plate)-nplates; ipl<=s0_plate; ipl++) {  //look for candidates in 4 plates
            ...
            for (int itrk1=0; itrk1<n_trk_cell; itrk1++) {
                ...
                dtx = start_segf->TX() - end_segf->TX();
                dty = start_segf->TY() - end_segf->TY();
                dtheta = TMath::Sqrt(dtx*dtx + dty*dty);
                if (b<MAX_B && b_back<MAX_B && dtheta<MAX_DT) {
...
int connect_tracks_secondstep() {
    ...
    TString file_name = Form("b%06i.0.%i.%i.trk_merged.root", IDBRICK, S0, SL);
    ...
    const int ADDED_GAPS = 0;

    // Merge Cuts
    float DT_MAX = 0.1, B_MAX=200;  //larger cuts than standard ones
    ...
    for (int itrk=0; itrk<N; itrk++) {
        ...
        else if (seg0->Plate()<START_PLATE_S2_ALT && segL->Plate()>START_PLATE_S2_ALT) {arrTRK_merged->Add(temptrack); n3 = n3 + 1;}
    ...
    TString merge_file_name = Form("b%06i.0.%i.%i.trk_merged_2.root", IDBRICK, S0, SL);
    if (EVERBOSE==100||EVERBOSE==101) merge_file_name = Form("b%06i.0.%i.%i.trk_merged_EVERBOSE_2.root", IDBRICK, S0, SL);
    ...
    //add already merged tracks in first step 
    int N_merged = arrTRK_merged->GetEntries();
    for (int itrk=0; itrk<N_merged; itrk++) {
        EdbTrackP *merged_track = (EdbTrackP*)arrTRK_merged->At(itrk);
        for (int k = 0; k<merged_track->N(); k++) {
                new((*segments_new_ordered)[k]) EdbSegP(*merged_track->GetSegment(k));
                new((*fitted_segments_new_ordered)[k]) EdbSegP(*merged_track->GetSegmentF(k));
            }

        nseg_new = merged_track->N();
        npl_new = merged_track->Npl();
        trid_new = merged_track->ID(); //trid is first track ID

            //trk_new->Copy(*s1_trk);
        trk_new = merged_track->GetSegment(0);
        trk_new->SetVid(0, tracks_new->GetEntries() ); 
        tracks_new->Fill();
