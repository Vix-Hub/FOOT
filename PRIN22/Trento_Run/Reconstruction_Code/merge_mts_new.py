...
outFile = r.TFile("mt.merged.root", "RECREATE")
...
for k1, iview in enumerate(actual_views):
	...
	if (SINGLE_VIEW>0 and iview!=SINGLE_VIEW): #option for debugging
		continue
	...
	if (iview%500==0):
		print(" Completed " + str(100.*iview/N) + " %, Time: " +str(time.time() - t0) + " s")
...
for entry in range(outTree.GetEntries()):
	...
	break_index = -1
	grains_x, grains_y, grains_z = outTree.grx, outTree.gry, outTree.grz
