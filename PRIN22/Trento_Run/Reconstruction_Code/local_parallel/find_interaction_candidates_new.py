...
for mt in mtracks:
    if (mt.is_dup==1 or mt.to_break==1):
        continue
    all_first_xyz.append([mt.hdid, mt.grx[0], mt.gry[0], mt.grz[0], mt.theta, mt.phi, mt.id2, mt.grx[mt.Ngr-1], mt.gry[mt.Ngr-1], mt.grz[mt.Ngr-1]])
    views.append(mt.hdid)
    if (mt.Ngr<MIN_NGR):
        continue
    elif (mt.len/mt.Ngr>MAX_LEN_OVER_NGR and mt.Ngr<30):
        continue
    selected_first_xyz.append([mt.hdid, mt.grx[0], mt.gry[0], mt.grz[0], mt.theta, mt.phi, mt.id2, mt.grx[mt.Ngr-1], mt.gry[mt.Ngr-1], mt.grz[mt.Ngr-1]])
    views.append(mt.hdid)
